...
import { Calendar, Clock, ChevronLeft, ArrowRight, Compass, Search, X } from 'lucide-react';
          ...
          >
            ...
            <div className="max-w-md mx-auto relative px-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Pesquisar matérias ou dicas..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-11 pr-10 py-3 rounded-2xl bg-white border border-stone-200 text-stone-850 placeholder-stone-400 focus:outline-hidden focus:border-brand-primary focus:ring-1 focus:ring-brand-primary/20 text-sm transition-all shadow-xs"
                />
                {searchQuery && (
                  <button
            ...
            {filteredPosts.length === 0 ? (
              <div className="text-center py-16 space-y-4 max-w-md mx-auto bg-stone-50/50 rounded-3xl border border-stone-100 p-8">
                <div className="w-12 h-12 bg-stone-100 rounded-full flex items-center justify-center mx-auto text-stone-400">
                  <Search className="w-6 h-6" />
                </div>
                <div className="space-y-1">
                  <h3 className="font-display font-bold text-base text-stone-850">Nenhuma matéria encontrada</h3>
                  <p className="text-xs text-stone-500 font-sans leading-relaxed">
                    Não encontramos resultados para "{searchQuery}". Tente usar palavras-chave diferentes como "casal", "orçamento" ou "itinerário".
                  </p>
                </div>
                <button
                  onClick={() => setSearchQuery('')}
                  className="mt-2 px-4 py-2 bg-stone-150 hover:bg-stone-200 text-stone-700 rounded-xl text-xs font-semibold font-display uppercase tracking-wider transition-colors cursor-pointer"
                >
                  Limpar Pesquisa
                </button>
              </div>
